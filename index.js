...
/**
 * Ideas and code from previous implementations:
...
const fround = Math.fround || ((array) => (x) => {
...
class MessagePack {
  ...
  static reallocate (length) {
    log('MessagePack: Setting buffer limit to', pb(length || Buffer.poolSize));
  ...
  static encode (value, persist) {
    ...
    switch (typeof value) {
      ...
      case 'object':
        ...
        if (ArrayBuffer.isView(value) === true) { // typedarrays and buffer
          if (value instanceof Buffer === false) { // typedarray to buffer
